...
const QueueManager = require(path.join(require.main.path, "public", "ytPlayback", "queueManager"));
...
async function playSong(guildId, song) {
  ...
  try {
    const songStream = await getYTDLStreamWithRetry(song.url);
...
async function getYTDLStreamWithRetry(url, retries = 3) {
  const streamOptions = {
    filter: "audioonly",
    quality: "highestaudio",
    highWaterMark: 1 << 26, // Adjusted buffer size
    requestOptions: {
      headers: {
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      const songStream = ytdl(url, streamOptions);
      await new Promise((resolve, reject) => {
        songStream.on('response', response => {
...
module.exports = { playSong, playNextSong, playSongWithRetry, ensurePlayerExists };
