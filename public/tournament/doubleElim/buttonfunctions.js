...
async function handleDoubleElimButtonPress(interaction, db) {
  db.read();
  const guildObject = await bot.guilds.cache.get(process.env.GUILD_ID);
  const reportChannel = await GetChannelByName(
    guildObject,
    "majordomo-logs-internal"
  );
  ...
  let doubleEliminationName = await db
    ...
    .value();
  let doubleElimination = await tournamentDetails[doubleEliminationName];
  ...
  if (splitButtonName[1] != "resetVote") {
    for (const match of doubleElimination.matches) {
    ...
    if (splitButtonName[2] > doubleElimination.matches.length) {
      console.log(
        "Button number: " +
          splitButtonName[2] +
          "\ndoubleElimination.matches.length: " +
          doubleElimination.matches.length
      );
      await interaction
        .reply({
    ...
    if (foundEntryAndVote[0]) {
      var changeVoteButtons = new ActionRowBuilder()
        .addComponents(
          new ButtonBuilder()
            //Button -> doubleElim-a-matchNumber
            .setCustomId(
              `doubleElim-resetVote-yes-${splitButtonName[1]}-${splitButtonName[2]}`
            )
            .setLabel("Yes")
            .setStyle("4")
        )
        .addComponents(
          new ButtonBuilder()
            .setCustomId(`doubleElim-resetVote-no-${splitButtonName[1]}`)
            .setLabel("No")
            .setStyle("1")
        );
      ...
      var removeVoteButtons = new ActionRowBuilder()
        .addComponents(
          new ButtonBuilder()
            .setCustomId(
              `doubleElim-resetVote-remove-${splitButtonName[1]}-${splitButtonName[2]}`
            )
            .setLabel("Yes")
            .setStyle("4")
        )
 ...
 **/
async function IsDoubleSecondVote(
  interaction,
  db,
  ...
  let doubleElimination = await tournamentDetails[currentTournamentName];
...
async function UpdateTotals(
  ...
  const matchObj = doubleElimination.matches.find(
